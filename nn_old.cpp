        ...
        for(size_t k = 0; k < Q; ++k)
        {
            ...
            for(size_t p = 0; p < S; ++p)
            {
				...
				for(size_t l = 0; l< X.cols(); ++l)
				{
					...
					(layers_[m-2].delta2)[l] = (
				(activation_secondgradient(layers_[m-2].a).asDiagonal()*layers_[m-1].W.transpose()*layers_[m-1].delta3[l]).array() *
					(layers_[m-2].W * layers_[m-3].da[l].transpose()).array()*0.0
	);

					(layers_[m-2].delta3)[l] = (layers_[m-1].W.transpose()*layers_[m-1].delta3[l]);
				}
			}
            ...
            for(size_t i = layers_.size() - 2; i > 0; --i)
            {
				...
				if(i > 1)
				{
					layers_[i-1].delta = activation_gradient(layers_[i-1].a).asDiagonal()*layers_[i].W.transpose()*layers_[i].delta;
					for(size_t l = 0; l< X.cols(); ++l)
					{
						
		            	layers_[i-1].delta += 
						activation_gradient(layers_[i-1].a).asDiagonal()*layers_[i].W.transpose()*layers_[i].delta2[l];

						(layers_[i-1].delta2)[l] = (
					(activation_secondgradient(layers_[i-1].a).asDiagonal()*layers_[i].W.transpose()*layers_[i].delta3[l]).array() *
						(layers_[i-1].W * layers_[i-2].da[l].transpose()).array()
	);

						(layers_[i-1].delta3)[l] = ((activation_gradient(layers_[i-1].a).asDiagonal()*layers_[i].W.transpose()*layers_[i].delta3[l]));
					}
				}
            }
        }
